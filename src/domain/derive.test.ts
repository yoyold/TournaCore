  ...
  describe('multi-stage tournaments', () => {
    ...
    it('feeds the qualifier result into the next stage', () => {
      ...
      const state = deriveTournamentState({
        tournament: tournament(4, [STAGE_A, STAGE_B]),
        stages: [qualifier, playoffs],
        // These stages state no arrangement, so they are drawn the way stages
        // created before the setting existed were: [1, 4, 3, 2]. The second
        // first-round match is therefore p3 against p2, with p3 on side A.
        matches: [
